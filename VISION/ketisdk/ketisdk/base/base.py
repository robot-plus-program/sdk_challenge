    ...
    def load_params(self, args):
        ...
        if self.default_args is not None:
            if self.args is None:
                ...
                self.args.merge_with(self.default_args)
        print(f'configs loaded ...')
        if not ProcUtils().isexists(self.cfg_path) and self.cfg_path is not None:
            self.args.write(self.cfg_path)
            print(f'Args was written on {self.cfg_path}')
