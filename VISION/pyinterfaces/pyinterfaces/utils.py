...
def visualize_insdetect(rgb, pred, show_steps=False, colors=None):
    ...
    for i,(lb, ins) in enumerate(pred.items()):
        num_ins = len(ins['boxes'])
        if num_ins==0:
            return out
        has_mask = False
        if 'masks' in ins:
            has_mask = ins['masks'] is not None
        out = show_masks_on_rgb(rgb=out, masks=ins['masks'], colors=[colors[i]]*num_ins) if has_mask else show_boxes_on_rgb(rgb=out, boxes=ins['boxes'])
...
def visualize_pred(rgb, pred, text=None):
    out = rgb.copy()
    if type(pred).__name__=='CustomInstances':
        out = pred.visualize(rgb=out)
    elif type(pred).__name__=='GraspGroup':
        out = pred.disp(rgb=out, topn=10)
        if text is not None:
            x,y = pred.centers_2d[0,:]
            out = show_text_on_rgb(out, text, (x,y), 0.6, (255,0,0), 1)
    elif isinstance(pred, dict):
        for k,v in pred.items():
            out = visualize_pred(out, v, text=k)
    else:
