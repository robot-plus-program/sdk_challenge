...
static const unsigned int INVALID_PARAMETER =
    0x8002; // At least one parameter provided in the command
// is invalid (or out of range) for the device
static const unsigned int INVALID_ADDRESS =
    0x8003; // An attempt was made to access a non existent address space location.
static const unsigned int WRITE_PROTECT = 0x8004; // The addressed register cannot be written to
static const unsigned int BAD_ALIGNMENT =
    0x8005; // A badly aligned address offset or data size was specified.
static const unsigned int ACCESS_DENIED =
    0x8006; // An attempt was made to access an address location which
...
static const unsigned int BUSY =
    0x8007; // A required resource to service the request isn't currently
// available. The request may be retried at a later time.
static const unsigned int NO_MSG = 0x800B; // Timeout, no message received
static const unsigned int PACKET_UNAVAILABLE =
    0x800C; // The request packet is not available anymore.
static const unsigned int INVALID_HEADER =
    0x800E; // The message header is not valid. Some of its fields do not
