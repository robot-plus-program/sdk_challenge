		...
		while (robotServer->connected)
		{
			...
			if (lenRecv > 0)
			{
				...
				if (robotServer->strRecv.compare("reqdata") == 0)
				{
					...
					robotServer->systemStat.sdata.tcp_pos[2] = static_cast<float>(robotServer->robotState.current_T_matrix[11]) * 1000 - 650;
	...
	while (ros::ok())
	{
		...
		while (robotServer->connected)
		{
			...
			if (lenRecv > 0)
			{
				...
				Position = robotServer->gripperState.width * 100000;
				// ROS_INFO("gripper width : %d", Position);
				// ROS_INFO("gripper state : %d", robotServer->gripperState.state);
